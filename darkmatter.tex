...
A flowchart of the programme used to evaluate the models can be found in \appendixref{cha:flowchart}.
...
Assume that the expected number of background events are $B \pm \sigma_B$ where $\sigma_B$ is the quadratic sum of the errors as explained in \subsectionref{subsec:errdata}. Also assume that the expected number of signal events are $S$, not to be confused with the variable $S$ in \sectionref{sec:smear}, assumed without fluctuation. 
