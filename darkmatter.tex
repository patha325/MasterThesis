...
A flowchart of how the programme used to evaluate the models can be found in \appendixref{cha:flowchart}.
...
W$\rightarrow \tau \nu$ \\ \hline
\end{tabular}
\caption{The main background processes from a collision. Each sample is a simulation of a physical process, the simulation names can be found in \appendixref{backpro}.}
\label{tab:backproc2}
\end{table}
...
Assume that the expected number of background events are $B \pm \sigma_B$ where $\sigma_B$ is the quadratic sum of the errors as explained in \subsectionref{subsec:errdata} and that the expected number of signal events is $S$, not to be confused with the variable $S$ in \sectionref{sec:smear}, assumed without fluctuation. 
...
Each of these models is modelled with a mass suppression scale, denoted $M$* and related to the nonrenormalizable of the effective theory \citep{Goodman:2010}, at 10000 GeV which is connected to the cross-section as shown in \eqref{eq:sigmas}. To study another $M$* the cross section has to be altered by using:
...
The choice of a new electron and muon veto require the comparison from \subsectionref{sec:sig:subsec:veri} to be redone. As seen in \tableref{tab:newcomp} the events for Z$\rightarrow\nu\nu$ and W$\rightarrow \mu\nu$ corresponded quite nicely to the values from the paper and are somewhat better than the previous comparison which can be seen in \subsectionref{Verifying background data}.
